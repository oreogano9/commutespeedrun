...
const DEFAULT_HIDDEN_RARITY_TIERS_BY_SKIN = Object.freeze({
  default: {},
  borderlands: {},
  borderlands2: {},
  minecraft: {},
...
function getOverlayRadiusForSkin(skin) {
  ...
  if (overlayRadiusBySkin && typeof overlayRadiusBySkin === "object") {
    return normalizeRadiusValue(overlayRadiusBySkin[normalizedSkin], overlayRadius);
  }
...
function getTierDisplayName(tierName) {
  const map = {
    ...
    seraph: "Seraph",
    "legendary-yellow": "Legendary (yellow)",
    "legendary-orange": "Legendary (orange)",
    "legendary-dark-orange": "Legendary (dark orange)",
...
function computeTierThresholds(sourceComments) {
  ...
  if (normalizeRaritySkin(raritySkin) === "borderlands") {
    return {
      common: 0,
      uncommon: MIN_BORDERLANDS_UNCOMMON_TIER_LIKES,
      rare: MIN_BORDERLANDS_RARE_TIER_LIKES,
      epic: MIN_BORDERLANDS_EPIC_TIER_LIKES,
      "legendary-yellow": MIN_BORDERLANDS_LEGENDARY_YELLOW_TIER_LIKES,
      "legendary-orange": MIN_BORDERLANDS_LEGENDARY_ORANGE_TIER_LIKES,
      "legendary-dark-orange": MIN_BORDERLANDS_LEGENDARY_DARK_ORANGE_TIER_LIKES,
      pearlescent: MIN_BORDERLANDS_PEARLESCENT_TIER_LIKES
    };
  }
...
function applyOverlayStyles() {
  ...
  overlayElement.style.setProperty("--overlay-pack-opacity", packOpacity);
...
async function getOverlaySettings() {
  ...
  raritySkin = normalizeRaritySkin(String(values?.raritySkin ?? DEFAULT_RARITY_SKIN));
...
function renderTimeMarkers() {
  ...
  entries.forEach(([time, groupedComments]) => {
    ...
    if (raritySkin === "default" && highestTierRank > 0) {
...
function showOverlay(comment, accentDelayMs = 0) {
  ...
  card.classList.add(
    isTierEffectsEnabled(tierRank, tierName) ? "tier-effects-enabled" : "tier-effects-disabled"
  );
  if (raritySkin === "default" && tierRank > 0) {
    if (tierName === "gold") {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "overlaySettings") {
    ...
    raritySkin = normalizeRaritySkin(String(message.raritySkin ?? raritySkin));
    overlayRadius = getOverlayRadiusForSkin(raritySkin);
    rarityLogicMode = normalizeRarityLogicMode(
      String(message.rarityLogicMode ?? rarityLogicMode)
    );
    hiddenRarityTiersBySkin = normalizeHiddenRarityTiersBySkin(
      message.hiddenRarityTiersBySkin ?? hiddenRarityTiersBySkin
    );
    commentScanStartDelaySec = clamp(
