...
function applyTierCardVisualStyle(card, tierConfig, tierRank = 0) {
  ...
  const skinStyle = getCurrentSkinStyleConfig();
  const styleFamily = String(getRaritySkinConfig()?.styleFamily || "glass").toLowerCase();
  const bodyRgb = rarityShared
    ? rarityShared.hexToRgb(tierConfig.bodyColor, "255 255 255")
    : "255 255 255";
  const borderRgb = rarityShared
    ? rarityShared.hexToRgb(tierConfig.borderColor || tierConfig.bodyColor, bodyRgb)
    : bodyRgb;
  const textColor = tierConfig.textColor || "#FFFFFF";
  const packOpacity = clamp(
    Number(skinStyle?.packOpacity ?? Number(overlayElement?.style?.getPropertyValue("--overlay-pack-opacity")) || 0.48),
    0.08,
    1
  );
  const borderEnabled = skinStyle?.borderEnabled !== false;
  const borderWidth = clamp(Number(skinStyle?.borderWidth ?? 1), 0, 12);
  const radius = clamp(
    Number(skinStyle?.radius ?? overlayRadius),
    MIN_OVERLAY_RADIUS,
    MAX_OVERLAY_RADIUS
  );
  const tierScale =
    Number.isFinite(Number(tierConfig?.tierScale)) && Number(tierConfig.tierScale) > 0
      ? Number(tierConfig.tierScale)
      : 1;

  let background;
  if (styleFamily === "flat" || styleFamily === "pixel") {
    background = `rgb(${bodyRgb} / ${packOpacity})`;
  } else if (styleFamily === "solid") {
    background = `rgb(${bodyRgb} / ${Math.min(1, packOpacity + 0.16)})`;
  } else {
    background = `linear-gradient(118deg, rgb(${bodyRgb} / ${Math.min(
      1,
      packOpacity * 0.75
    )}), rgb(255 255 255 / ${Math.min(
      0.24,
      packOpacity * 0.32
    )}) 45%, rgb(${bodyRgb} / ${Math.min(
      1,
      packOpacity * 0.9
    )})), rgb(20 20 20 / var(--overlay-bg-alpha))`;
  }

  card.style.setProperty("--overlay-tier-scale", String(tierScale));
  card.style.background = background;
  card.style.color = textColor;
  card.style.borderRadius = `${radius}px`;
  card.style.borderStyle = borderEnabled ? "solid" : "none";
  card.style.borderWidth = borderEnabled ? `${borderWidth}px` : "0";
  card.style.borderColor = borderEnabled ? `rgb(${borderRgb} / 0.9)` : "transparent";
  card.style.boxShadow =
    styleFamily === "flat" || styleFamily === "pixel"
      ? `0 8px 24px rgb(0 0 0 / var(--overlay-shadow-alpha))`
      : `0 10px 30px rgb(0 0 0 / var(--overlay-shadow-alpha))`;

  card.classList.add("tier-effects-disabled");
  card.classList.remove("effect-glow", "effect-pulse", "effect-rainbow-cycle", "effect-sheen");
  getTierEffects(tierConfig, tierRank).forEach((effect) => {
...
function applyTierMarkerVisualStyle(marker, tierConfig, tierRank = 0) {
  ...
  const width = clamp(Number(tierConfig.markerWidthPx || 5), 3, 30);
  const height = clamp(Number(tierConfig.markerHeightPct || 120), 80, 420);
  const offset = clamp(Number(tierConfig.markerOffsetTopPct || -10), -180, 40);
  marker.style.width = `${width}px`;
  marker.style.minWidth = `${width}px`;
  marker.style.height = `${height}%`;
  marker.style.top = `${offset}%`;
  marker.style.background = tierConfig.markerColor || tierConfig.bodyColor || "#FFFFFF";
  marker.style.borderRadius = "999px";
  marker.classList.remove("effect-glow", "effect-pulse", "effect-rainbow-cycle", "effect-sheen");
  getTierEffects(tierConfig, tierRank).forEach((effect) => {
    marker.classList.add(`effect-${effect}`);
  });
}
...
function getTierRank(likesCount) {
  ...
  if (popularityModeEnabled) {
    ...
    for (let index = tierKeys.length - 1; index >= 1; index -= 1) {
      ...
      if (likesCount >= getTierThresholdByKey(tierKey)) {
        return index;
      }
    }
    return 0;
  ...
  if (likesCount >= nonPopularityEliteThreshold) {
    return 2;
  }
  if (likesCount >= nonPopularityPrimaryThreshold) {
    return 1;
  }
  return 0;
}
