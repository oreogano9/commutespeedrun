...
function applySettingHoverDescriptions() {
  const tooltips = {
    ...
    "popularity-mode-toggle": {
      ...
      description: "Enables Silver, Gold, Platinum, Diamond, Purple, and Ruby tiers."
    },
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function showTopLikedCutoffPreview(thresholdPercent) {
    ...
    if (goldResponse?.ok) {
      ...
      const flooredGoldCutoff = Math.max(MIN_GOLD_TIER_LIKES, goldCutoff);
      const flooredDiamondCutoff = Math.max(
        MIN_DIAMOND_TIER_LIKES,
        diamondCutoff,
        flooredGoldCutoff + MIN_DIAMOND_TIER_GAP_FROM_GOLD
      );
      const goldText = `Gold cutoff: ${flooredGoldCutoff}+ likes`;
      thresholdPreviewBadge.textContent = goldText;
      if (topLikedCutoffInline) {
        const diamondText = `Diamond cutoff: ${flooredDiamondCutoff}+ likes`;
